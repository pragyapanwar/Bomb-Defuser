...
<link   href="css/question4QA.css"  rel="stylesheet">
<link href="css/question4.css" rel="stylesheet" type="text/css">
...
<body style="background-image: url(Image/ques4.jpg);background-repeat: no-repeat;height:100px;width:1349px;" onload = "startTimer();mainTextScroller();noBack();" onpageshow="if (event.persisted) noBack();" onUnload="">
    <div class="menu">
        <br>
...
//        out.println(" yes."+timer);
  ...
  <a><form method="post" action="InstrTime.jsp" onclick="mytime()">
            <input type="hidden" name="time" id="demoinst" value="">
            <input type="submit" class="homebutt1" value="Instructions">
    </form></a>
...
<center><div class="level">Level 4</div></center>

<p><a href="javascript:void(0)" style="margin-bottom:-10px;" onclick="clickinc();document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block'"><img id="boba" src="Image/img1.png"/></a>
  </p>
    <div id="light" class="white_content">A “drop” consists of taking a single grenade in the elevator up to a particular floor, dropping the grenade from that floor, taking the elevator back down, and checking how much destruction  the dropped grenade has caused.
<a href="javascript:void(0)" onclick="document.getElementById('light').style.display='none';document.getElementById('fade').style.display='none'"style="text-decoration:none;font-weight:bold;color:#000000;font-size:30px;position:absolute;right:30px;top:20px">X</a>
  </div>
...
Sid asks the BOMB DEFUSAL HEAD QUARTER team to seek help from the Indian military to capture the entire terrorists group. For that purpose they are having  a military training session in which they are testing the impact of the explosion after dropping the bombs from different heights.  You have 2 Scalable Offensive Hand Grenade, and you need to determine the highest floor in a building from which a grenade may be dropped without exploding.  Both of the hand grenades are identical .
<br>* If a grenade is dropped from a given floor and explodes, then any other grenade will also explode if dropped from that or any higher floor.
</br>*If a grenade is dropped from a given floor and does not explode, then neither it nor any other grenade will explode if subsequently dropped from that or any lower floor.
<br>A grenade may be dropped any number of times until it explodes. 
</br>But once its explodes, it is unusable and cannot be dropped again.
If the building has floors F = 100  what is the least number of drops ‘D’ required to guarantee that after at most ‘D’ drops you can determine the highest floor from which a grenade may be dropped without exploding?
        ...
        <tr>
            <td>
             <div id="keyboard" >
                   <form action="answer.jsp" method="post" onclick="clickval();mytime();">
            <input type="hidden" name="time" id="demo3" value="">
            <input type="hidden" name="click" id="passclick" value="">
                <div>   
 ...
 <input style="width:55px;height:35px;margin-left:15px;" id="accept" type="submit" name="enter" value="enter">
  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      <p>Question would give you an integer value on solving it and the sum of those integer values obtained on solving would give the code</p>
...
//          document.getElementById('bgvid').style.display = "block";
       ...
       if(passon<21)
       {
          document.getElementById('digit-5').style.backgroundColor   = "red";
          document.getElementById('digit-4').style.backgroundColor   = "red";
          document.getElementById('digit-3').style.backgroundColor   = "red";
          document.getElementById('digit-2').style.backgroundColor   = "red";
          document.getElementById('digit-1').style.backgroundColor   = "red";
          document.getElementById('digit-0').style.backgroundColor   = "red";
       }
...
function mytime() {
    ...
    document.getElementById("demo3").value=passon;
     ...
     document.getElementById("demoinst").value=passon;
...
function clickval()
{ 
    document.getElementById("passclick").value=click;
}
</script>
<script>
// Get the modal
